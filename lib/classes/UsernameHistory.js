/**  @class
 * @desc Represents the username history of a Minecraft account
 */
class UsernameHistory {

  constructor(history) {
    this._history = new Map()
    history.forEach(el => {
      this._history.set(el.name, el.changedToAt || -Infinity)
    })
  }

  all = () => this._history
  asArray = () => [...this._history.values()]

  first = () => this.asArray().find(user => {
    if (user === -Infinity) return user
  })

  last = () => {
    let last = this.first()

    const usernames = this._history.keys()
    const len = usernames.length

    for (let i = 0; i < len; i++) {
      const username = usernames[i]
      if (this.get(username) >= this.get(last))
        last = username
    }

    return last
  }

  get = username => this._history.get(username)
  had = username => this._history.has(username)
  
  at = timestamp => {
    let mostRecentUser = this.first()

    const usernames = this._history.keys()
    const len = usernames.length

    for (let i = 0; i < len; i++) {
      const username = usernames[i]
      const historicUser = this.get(username)

      const withinTimestamp = historicUser <= timestamp
      const moreRecent = historicUser >= this.get(mostRecentUser)

      if (withinTimestamp && moreRecent)
        mostRecentUser = username
    }

    return mostRecentUser
  }
}

module.exports = UsernameHistory
